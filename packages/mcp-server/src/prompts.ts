    ...
    ({ widget, states, viewports }) =>
      userMessage(
        [
          ...
          "",
          "1. Discover before generating anything: the Flutter command (use `fvm flutter` when the repo has `.fvmrc`), the SDK version, the widget's constructor, and the theme, localization delegates, providers or inherited widgets, router and media dependencies, fonts, and assets it needs. Reuse existing test wrappers, builders, and fixtures; do not create a second app or test convention.",
          "2. Pick one lane, in this order:",
          "   a. Widget Previewer, for interactive visual iteration when the SDK supports `flutter widget-preview start`. Write a temporary `@Preview` adapter that wraps the widget in the app's real theme, localizations, and inherited dependencies.",
          "   b. Widget-test capture harness, for deterministic state matrices, exact logical sizes, overflow detection, and semantics checks. Write a temporary test that sets the view size and DPR, pumps each state, and fails on overflow or any exception from `tester.takeException()`.",
          "   c. Temporary web entrypoint, only when the previewer is unavailable or cannot host the widget. Serve it on loopback with `flutter run -d web-server`.",
          ...
          '4. Render visual lanes inside Pickforge, never on the user\'s real display. For the previewer, create a session with `session_create` (type "desktop") and start the previewer with `desktop_launch` so its browser window opens on the isolated display. For the web entrypoint, open it in that session or a `session_create` (type "browser") session. Capture each state and viewport with `desktop_screenshot`, using a runSlug that names the state and width, and inspect every image.',
          "5. For each state, exercise text scaling (at least 1.0 and 2.0), semantics labels, the tap target and text contrast guidelines (`meetsGuideline`), overflow and exceptions, and every responsive breakpoint the widget has. Use real project fonts and assets. Report rendering that is not deterministic or that differs between the widget-test and Chromium engines.",
          ...
          "8. Confirm the source tree is clean with `git status --short`, compared with its state before you started. Then report evidence for each requested state and viewport: screenshot paths or run ids, check results, and anything you could not verify. Use `artifact_report` for captured runs and record the verdict with `evidence_outcome`.",
          "",
          HUMAN_BLOCKER_GUIDELINE,
        ].join("\n"),
      ),
