    ...
    ({ widget, states, viewports }) =>
      userMessage(
        [
          ...
          "8. Confirm `git status --short` matches the baseline. Then report evidence for each requested state and viewport: screenshot paths or run ids, check results, and anything you could not verify. Use `artifact_report` for the session's evidence run and record the verdict on that run with `evidence_outcome`, listing the inspected screenshots.",
          "",
          HUMAN_BLOCKER_GUIDELINE,
        ].join("\n"),
      ),
