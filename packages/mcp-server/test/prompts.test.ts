...
describe("prompts", () => {
  ...
  it("guides an isolated flutter component preview", async () => {
    ...
    expect(text).toMatch(/Record `git status --short` as the baseline\. Then discover before generating anything/);
    expect(text).toMatch(/a\. Widget Previewer, preferred for interactive[\s\S]*b\. Widget-test capture harness, always used for the checks[\s\S]*c\. Temporary web entrypoint, only when/);
    expect(text).toContain("flutter widget-preview start --web-server");
    expect(text).toContain("flutter run -d web-server");
    expect(text).toContain("@Preview");
    expect(text).toContain("tester.takeException()");
    expect(text).toMatch(/session_create` \(type "browser"\) session, which has a private profile/);
