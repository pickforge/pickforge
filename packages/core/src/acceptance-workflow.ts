/** Shared by MCP instructions, the device_pass prompt, and agent installs. */
export const DEVICE_PASS_WORKFLOW = `Test the changed user journey in a rendered Pickforge session.
Begin an evidence run per scenario by creating a fresh session with session_create and evidence recording enabled; retain its runId from the tool results or artifact_list.
...
Save screenshots of meaningful intermediate and final states with the lab screenshot tools, retaining the exact screenshots/<id>.png paths from that run.
...
Use only the exact screenshots/<id>.png paths from that run in inspectedScreenshots, and only for images you actually inspected; record unknown revision as unknown.
