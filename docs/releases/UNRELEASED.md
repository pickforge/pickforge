...
- `artifacts report --finalize-orphans` and MCP `artifact_report` with `finalizeOrphans: true` recover interrupted evidence in place, reconcile manifest artifacts, and create a session index across short-lived processes. Stop producers first. Orphaned runs are labeled explicitly; journals are never rewritten or deleted by recovery.
