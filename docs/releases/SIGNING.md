...
- **Why not sign yet.** Developer ID signing and notarization need a paid Apple
  ...
  deliberate cost decision for the 0.4, 0.5.0 and 0.6.0 trains, not an
  oversight.

## Decision for 0.4.0, 0.5.0 and 0.6.0

Ship unsigned and un-notarized for the 0.4 train, for 0.5.0 and for 0.6.0, with
checksums, npm provenance, and this document. 0.6.0 continues the existing
ad-hoc signed, not notarized, no Developer ID policy. Revisit before any release
that advertises macOS as a first-class installed product rather than a verified
CLI download.
