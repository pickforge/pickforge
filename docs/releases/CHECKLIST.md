...
grep -n -i -E 'to be updated|not executed|TODO|section is (still )?a draft|^[[:space:]#>*_-]*draft([[:space:]]+(release[[:space:]]+)?(notes|instructions|placeholder|copy)|[[:space:]]*[-:][[:space:]]|[[:space:]]*$)|drafting[[:space:]]+(notes|instructions|placeholder|copy)' docs/releases/UNRELEASED.md   # must print nothing
