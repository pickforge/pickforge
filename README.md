...
- `report.html` — escaped human viewer generated at finalization: device and
  outcome summary, device/scenario filters, and a capture inspection view, all
  usable with scripts blocked
...
- Evidence timelines persist only allowlisted metadata; typed values become length/type metadata, and network headers, bodies, and URL queries are dropped. Static HTML reports escape page-controlled text and use a no-network CSP that admits exactly one inline script by sha256 hash — text search and arrow-key browsing only; every control also works with scripts blocked.
