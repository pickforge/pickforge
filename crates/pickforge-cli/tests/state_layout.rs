...
/// The entry-name table both tools implement. `packages/core/test/
/// state-layout.test.ts` pins the TypeScript side against the same rows;
/// changing one without the other splits ownership.
#[test]
fn entry_ownership_matches_the_documented_table() {
    let rows: &[(&str, Owner)] = &[
        ("layout.json", Owner::Shared),
        ("project.json", Owner::Integration),
        ("project.json.pickforge-backup-20260101", Owner::Integration),
        (".pickforge-tmp-abcd", Owner::Transient),
        (".pickforge-tmp-layout-1-2", Owner::Transient),
        ("runs", Owner::Lab),
        ("sessions", Owner::Foreign),
        ("config.json", Owner::Foreign),
        ("notes.txt", Owner::Foreign),
        ("", Owner::Foreign),
        ("runs2", Owner::Foreign),
        ("project.json.bak", Owner::Foreign),
    ];
    for (name, expected) in rows {
        assert_eq!(state::classify_entry(name), *expected, "entry {name:?}");
    }
}
